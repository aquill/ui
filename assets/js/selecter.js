        ...
        if (!$select.hasClass("selecter-element")) {
            ...
            var $selecter = $select.next(".selecter"),
                data = $.extend({
                    ...
                    $wrapper: $selecter.find(".selecter-wrapper"),
                    $selectOptions: $selecter.find(".selecter-options"),
                    index: originalIndex,
                    guid: guid++
                }, opts);
    ...
    function _buildOptions(data) {
        ...
        for (var i = 0, count = data.$allOptions.length; i < count; i++) {
            ...
            if ($op[0].tagName == "OPTGROUP") {
                html += '<dt class="selecter-group';
                // Disabled groups
                if ($op.is(":disabled")) {
                    html += ' disabled';
                }
                html += '">' + $op.attr("label") + '</dt>';
            } else {
                var opVal = $op.val();
                ...
                html += '><i>' + $("<span></span>").text( _checkLength(data.trimOptions, $op.text()) ).html() + '</i></' + itemTag + '>';
                j++;
            }
        }
        
        data.$selectOptions.html(html);
